from DataModels.Datamodels import ScoreBase, WTCScoreSystem, Player, Game, GameStatuses, Round, RoundStatuses
from PairingLibrary.PairingManager import SwissPairingManager
from PairingLibrary.Tournament import Tournament, ScoreEnum, PairingEnum
...
class TournamentTester(unittest.TestCase):
    ...
    def test_set_pairing_method(self):
        TournamentTester.t.set_pairing_manager(PairingEnum.Swiss)
        self.assertIsInstance(TournamentTester.t.pairing_manager, SwissPairingManager)

    def test_register_player(self):
        TournamentTester.t.register_player('Steve', ScoreEnum.WTC)
    ...
    def test_round_counting_up(self):
        self.assertEqual(TournamentTester.t.round_number, 1)

    def test_round_end_returned_type(self):
        self.assertIsInstance(TournamentTester.t.end_round(), List)
